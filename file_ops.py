...
def _get_message_components(event: Any) -> list[Any]:
    ...
    # Discord 适配器通常会把附件保留在 message_obj.attachments 或 raw_message.attachments。
    # 附件字段常见形态：url/proxy_url/filename/content_type/size。
    attachments = getattr(message_obj, "attachments", None)
    if attachments:
        components.extend(list(attachments))

    raw_message = getattr(message_obj, "raw_message", None)
    if isinstance(raw_message, dict):
        raw_attachments = raw_message.get("attachments") or raw_message.get("files")
