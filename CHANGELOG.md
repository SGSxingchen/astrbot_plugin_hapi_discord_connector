...
1. 待审批提醒改为仅统计仍处于 active 状态的 HAPI 原生权限请求；发送前会再次查询会话详情，自动清理已处理、不存在或 inactive 会话中的陈旧请求。
2. 以 `session_id + request_id` 作为稳定键维护请求，自动审批成功后立即清理本地项；本地 LLM/压缩临时审批不再污染 HAPI 提醒计数或触发跨窗口提醒。
3. 同一待审批集合增加节流，集合未变化时不会被后续请求重新武装为重复提醒；插件重复启动不会丢失已登记后台任务的取消句柄，销毁时逐一取消并等待完成。
4. 待审批提醒 Embed 使用独立样式，不再把正文再次复制到“参数 / 详情”。


## v2.1.0 — 受控 cron 会话直连

1. 新增默认关闭的后台 session_id 直连模式。只有被精确加入白名单的 AstrBot cron 任务，才能通过完整 session ID 或唯一前缀查询状态、读取历史和续派消息。
2. 后台目标必须已经持久化绑定到仍在线的 Discord 窗口；不会按默认窗口、agent 类型或任意全局 session 猜测目标。
3. 非 yolo 续派会把单张审批卡稳定路由到该 session 首个仍在线的 Discord 绑定窗口；yolo 或全局自动审批直发也必须由独立配置显式开启。
4. 后台模式不暴露或执行停止、归档、删除、创建、加入/退出和配置变更等危险操作。
5. 统一插件注册版本与 metadata 为 v2.1.0，并新增后台直连配置说明与回归测试。

## v1.3.12 — 审批卡保留 Markdown 可读性

1. LLM 工具审批卡和单项审批结果不再把参数转成 Python 字典字符串。
2. 发送消息审批保留原任务的 Markdown 与换行；创建会话审批以模型、强度、目录、模式等中文字段展示。
3. LLM 工具结果卡隐藏内部频道标识与 Request ID，改为显示面向操作的来源和内容。
