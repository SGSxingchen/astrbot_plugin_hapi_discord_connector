...
- **审批面板**：选择 pending request 后批准、拒绝、批准全部、刷新、返回；LLM 工具审批通知优先使用 Embed，异常时保留 pending 并降级提示
...
操作类工具的审批入口与文本流分离，统一走 `/dhapi` 卡片完成确认。

---

### Discord Embed 兼容

LLM 工具审批通知会优先发送 Discord Embed，保留卡片式审批入口。插件层提供本地兼容组件与发送补丁，避免 AstrBot DiscordEmbed 包装类在热重载或字段定义不一致时导致 `no field "title"`。
...
| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| `auto_approve_enabled` | 自动审批开关：开启后全天候生效，自动批准非交互式权限请求 | 关闭 |
