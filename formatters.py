...
def format_directory(
    ...
    """格式化目录浏览（/dhapi files 返回结果），目录在前文件在后"""
    if not entries:
    ...
    lines.append("")
    lines.append("💡 /dhapi files <文件夹> — 查看子目录")
    lines.append("💡 /dhapi find <关键词> — 搜索文件")
    lines.append("💡 /dhapi dl <路径> — 下载文件")
    lines.append("💡 /dhapi upload — 上传文件")
    return "\n".join(lines)


def format_file_search(files: list[dict], query: str) -> str:
    """格式化文件搜索结果（/dhapi find 返回结果）"""
    if not files:
...
HELP_COMMANDS = [
    ...
    {
        "topic": "push",
        "usage": "/dhapi bind [claude|codex|gemini]",
    ...
    },
    {
        "topic": "session",
        "usage": "/dhapi",
        "summary": "中断 session（默认当前，别名: /dhapi stop）",
        "example": "/dhapi",
        "home": True,
    },
    {
    ...
    {
        "topic": "session",
        "usage": "/dhapi resume [序号|ID前缀]",
        "summary": "恢复被 archive 的 inactive session",
        "example": "/dhapi resume 1",
        "home": True,
    },
    {
        "topic": "session",
    ...
    {
        "topic": "session",
        "usage": "/dhapi clean [路径前缀]",
        "summary": "批量清理 inactive sessions",
        "example": "/dhapi clean C:/work/project",
        "home": False,
    },
    {
        "topic": "chat",
    ...
    {
        "topic": "chat",
        "usage": "/dhapi to <序号> <内容>",
        "summary": "发送到指定 session",
        "example": "/dhapi to 2 继续上一个任务",
        "home": False,
    },
    {
        "topic": "chat",
    ...
    {
        "topic": "files",
        "usage": "/dhapi files [路径]",
        "summary": "浏览远端目录",
        "example": "/dhapi files src",
        "home": True,
    },
    {
        "topic": "files",
        "usage": "/dhapi files -l [路径]",
        "summary": "浏览目录并显示文件大小",
        "example": "/dhapi files -l .",
        "home": False,
    },
    {
        "topic": "files",
        "usage": "/dhapi find <关键词>",
        "summary": "搜索远端文件",
        "example": "/dhapi find config",
        "home": True,
    },
    {
        "topic": "files",
        "usage": "/dhapi download <路径>",
        "summary": "下载远端文件到聊天（别名: /dhapi dl）",
        "example": "/dhapi dl logs/app.log",
        "home": True,
    },
    {
        "topic": "files",
        "usage": "/dhapi upload [cancel]",
        "summary": "上传文件到当前 session，支持快捷前缀附件",
        "example": "/dhapi upload\n> 分析这张图 [附带图片]",
        "home": True,
    },
    {
        "topic": "config",
    ...
    {
        "topic": "config",
        "usage": "/dhapi output [级别]",
        "summary": "查看或切换推送级别",
        "example": "/dhapi output summary",
        "home": True,
    },
    {
        "topic": "config",
...
def format_unknown_command_help(command: str) -> str:
    ...
    if normalized == "reset":
        return "命令已调整为: /dhapi bind reset"
    lines = [
        f"未知命令: /dhapi {command}",
        "",
        "💡 按功能查看帮助：",
        "  /dhapi help 会话    会话管理",
        "  /dhapi help 对话    对话与消息",
        "  /dhapi help 审批    审批权限请求",
        "  /dhapi help 通知    通知与路由",
        "  /dhapi help 文件    文件操作",
        "  /dhapi help 配置    配置管理",
        "",
    ...
    if matches:
        ...
        for item in matches:
            ...
            if summary:
                lines.append(f"  /dhapi {item}  {summary}")
            else:
                lines.append(f"  /dhapi {item}")
    return "\n".join(lines)
    ...
    if topic == "push":
        lines.extend(
            [
                ...
                "  2. 没有绑定时，如果配置了模型默认窗口，例如 /dhapi bind codex，就发到那个窗口。",
                "  3. 还没有时，发到 /dhapi bind 设置的默认窗口。",
                "",
                "相关命令：",
                "  /dhapi bind               设置默认通知窗口",
                "  /dhapi bind codex         设置 Codex 默认通知窗口",
                "  /dhapi bind status        查看当前通知配置",
                "  /dhapi bind reset         清除 session 绑定和窗口状态，不清除默认窗口配置",
                "",
            ]
        )
...
def _get_home_help_text() -> str:
    ...
    lines = ["HAPI Discord Connector 常用命令帮助", ""]
    ...
    for topic_key, topic_label in HELP_TOPICS:
        lines.append(f"/dhapi help {topic_key}    {topic_label}")
    return "\n".join(lines).rstrip()
...
def get_help_embed_data(topic: str = "") -> tuple[str, str, list[dict]]:
    """返回 Discord Embed 帮助页数据：每个分类一个 field。"""
    normalized = _normalize_help_topic(topic) or "home"
    title = "HAPI Discord Connector 帮助"
    description = (
        "仅支持 Discord。主命令 `/dhapi`，快捷前缀默认 `>`；不支持戳一戳审批。"
    )

    topics = ["session", "chat", "approve", "push", "files", "config"]
    if normalized not in ("home", "all"):
        topics = [normalized]
        title = f"HAPI Discord Connector 帮助 / {HELP_FIELD_TITLES.get(normalized, normalized)}"
    ...
    for topic_key in topics:
        ...
        for item in HELP_COMMANDS:
            if item["topic"] != topic_key:
                continue
            if normalized == "home" and not item.get("home"):
                continue
            usage = item["usage"].replace("/dhapi", "/dhapi")
        ...
        if lines:
            ...
            fields.append(
                {
                    "name": HELP_FIELD_TITLES.get(topic_key, topic_key),
                    "value": value,
                    "inline": False,
                }
            )
    ...
    fields.append(
        {
            "name": "专题",
            "value": "`/dhapi help 会话` `/dhapi help 对话` `/dhapi help 审批` `/dhapi help 通知` `/dhapi help 文件` `/dhapi help 配置` `/dhapi help 全部`",
            "inline": False,
        }
    )
